...
def filter_cars_by_criteria(incoming_dict: dict, year: int, engine_capacity: float, price: int, **kwargs) -> dict:
  ...
  if not kwargs.get("name"):
    result_dict: dict = {
      ...
      value[1] >= search_criteria[0] and value[2] >= search_criteria[1] and value[4] <= search_criteria[2]
    }
  else:
    result_dict: dict = {
      car: value for car, value in incoming_dict.items() if
      value[1] >= search_criteria[0] and value[2] >= search_criteria[1] and value[4] <= search_criteria[2] and car==kwargs.get("name")
    }
  return result_dict

print(
  filter_cars_by_criteria(car_data, *search_criteria, name="Mazda")
)